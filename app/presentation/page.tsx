...
export default function PresentationPage() {
  ...
  const slides: Slide[] = [
    ...
    // 2. Executive Summary
    {
      ...
      subtitle: "Strategic Vision & Urban Transport Scope",
      content: (
        <div className="p-6 border border-surface rounded-2xl bg-card space-y-3 shadow-soft max-w-xl mx-auto">
          <h3 className="text-lg font-bold font-display text-primary">Strategic Goal</h3>
          <p className="text-sm text-foreground-secondary leading-relaxed">
            To provide Lagos State Ministry of Transportation delegates with a unified presentation portal communicating commuter flow insights, farebox digital adoption, and corridor congestion fixes.
          </p>
        </div>
      )
    },
    ...
    // 11. Power BI Dashboards
    {
      ...
      content: (
        <div className="p-6 border border-surface rounded-2xl bg-card text-center space-y-3 max-w-xl mx-auto">
          <Icons.powerbi className="h-10 w-10 text-primary mx-auto animate-pulse" />
          <h4 className="font-bold font-display text-base">Power BI Embedded Integration</h4>
          <p className="text-xs text-foreground-secondary max-w-md mx-auto">
            Team DixNova&apos;s interactive Power BI reports are embedded directly into the platform shell with role-based security filters.
          </p>
        </div>
      )
    },
  ...
  return (
    <div className="h-screen max-h-screen w-screen max-w-vw overflow-hidden bg-background text-foreground flex flex-col justify-between p-4 md:p-8">
      {/* Top Header Controls */}
      <div className="flex items-center justify-between border-b border-surface pb-3 flex-none">
        <div className="flex items-center gap-3">
          <Badge variant="default" className="text-xs">{slide.stage}</Badge>
          <span className="text-xs font-mono text-foreground-secondary">SLIDE {currentSlide + 1} / {slides.length}</span>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center items-center overflow-hidden my-auto py-2">
        {currentSlide !== 0 && (
          <div className="space-y-2 text-center mb-6">
            <h1 className="text-3xl md:text-4xl font-display font-extrabold tracking-tight text-foreground">
              {slide.title}
            </h1>
            <p className="text-sm text-foreground-secondary">{slide.subtitle}</p>
          </div>
        )}
      ...
      <div className="flex items-center justify-between border-t border-surface pt-3 max-w-3xl mx-auto w-full flex-none">
        <Button
          variant="outline"
          size="sm"
